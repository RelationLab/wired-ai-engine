...
def create_msg(tables, question, history):
    messages = [
        SystemMessage(content="You are a PostgreSQL expert. Determine whether the user needs to generate a query sql based on the user's input question. If so, create a syntactically correct PostgreSQL query statement and output the response using "
                              ...
                              "name in the table. For example, The user's question is 'EBTC' but the database stores 'eBitcoin', you can use the get_master_data function to get the exact value of the asset stored in the database.When you get more "
                              "than or equal to 2 exact value of the asset, Please ask a rhetorical question to confirm which one should be selected.\nIf the user needs to generate SQL you can use the get_sql_sample function to get the SQL "
                              "statement for a similar question to help you understand the user's table structure and data characteristics.\nWhen a percentage occurs, such as 0.5%, it corresponds to 0.005, and your understanding of it should be "
                              "based on rules such as these. Finally, could you please take the generated sql as a whole and de-emphasize the address field by group by, e.g. select address from (select address from table_name) a1 group by "
                              "address."),
