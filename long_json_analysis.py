...
The details of the on-chain assets currently held by this address or set of addresses, including asset balance, transaction volume, and trading count (activity), cover a variety of on-chain assets like UNI, CryptoPunks, ENS, etc. (The balance/transaction volume/activity of DeFi, NFT, and Web3 assets are calculated separately). when the value of the 'type' field  is 'token' and the 'group' field is ('Balance' or 'Volume'), the corresponding data unit is USD;If the value of the 'group' field is 'Activity', the corresponding data unit is 'count'.when the value of the 'type'  field  is  ('web3' or 'nft'),  and the 'group' field is ('Balance' or 'Volume' or  'Activity')   the corresponding data unit is 'count'.
""")
